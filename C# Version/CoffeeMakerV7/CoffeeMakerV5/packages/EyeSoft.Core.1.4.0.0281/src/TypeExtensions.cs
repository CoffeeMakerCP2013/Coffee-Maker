namespace EyeSoft
{
	...
	public static class TypeExtensions
	{
		...
		{
			return
				type.IsOneOf(types.AsEnumerable());
		}

		public static bool IsOneOf(this Type type, IEnumerable<Type> types)
		{
			return
				types.Any(type.EqualsOrSubclassOf);
		}
		...
		public static PropertyInfo GetAnyInstanceProperty(this Type type, string propertyName)
		{
			return
				type
					.GetProperty(propertyName, InstanceBindingFlags);
		}
		...
		public static IEnumerable<MemberInfo> GetAnyInstanceFieldsOrProperties(this Type type)
		{
			var members =
				type
					.GetMembers(InstanceBindingFlags).AsEnumerable();
			...
			members =
				members
					.Where(member => member.GetAttribute<CompilerGeneratedAttribute>().IsNull());
		...
		{
			return
				typeToInspect.RelatedTypes(new HashSet<Type>(), type => true);
		}

		public static IEnumerable<Type> RelatedTypes(this Type typeToInspect, Func<Type, bool> predicate)
		{
			return
				typeToInspect.RelatedTypes(new HashSet<Type>(), predicate);
		}
		...
		{
			Ensure.That(interfaceType.IsInterface).Is.True();
		...
		public static bool IsEnumerableOf<TInterface>(this Type type)
			where TInterface : class
		{
		...
		{
			return
				EqualsOrSubclassOf(type, typeof(T), true);
		}
