	...
	public static class Reflector
	{
			...
			this Expression<Func<T, TProperty>> expression)
		{
			...
			var propertyNotFoundMessage =
				"Cannot find a instance property with name {Name} on type {Type}."
					.NamedFormat(memberOfDeclaringType.Name, typeof(T).FullName);

			var exception = new ArgumentException(propertyNotFoundMessage);

			Ensure
				.That(memberOfReflectType)
		...
		public static Type GetPropertyOrFieldType(this MemberInfo propertyOrField)
		{
			...
			throw
				new ArgumentOutOfRangeException(
					"propertyOrField",
					"Expected PropertyInfo or FieldInfo; found {0}".NamedFormat(propertyOrField.MemberType));
		}
	}
}